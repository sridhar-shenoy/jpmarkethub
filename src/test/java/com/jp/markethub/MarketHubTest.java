...
public class MarketHubTest {
    ...
    private List<JpInternalConsumer> consumers;
    ...
    public void testBidAndOfferAndLastPriceUpdateToSingleConsumer() throws Exception {
        try (JpInternalConsumer jpStride = new JpInternalConsumer(BID_OFFER_LASTPRICE_INTERNAL_PORT, "JP Stride")) {

            jpStride.connectToMarketHubAndListen();
            jpStride.awaitConnectionToMarketHub(1, TimeUnit.SECONDS);

            //-- Send initial data Exchange
    ...
    public void testBidAndOfferUpdateOnlyToSingleConsumer() throws Exception {
        try (JpInternalConsumer jpStride = new JpInternalConsumer(BID_OFFER_LASTPRICE_INTERNAL_PORT, "JP Stride")) {
            ...
            waitTillTrue(() -> hub.getConsumerCount() == 1, 1, TimeUnit.SECONDS);
            bidOfferProducer.publish("4,103.0,104.0\n");
    ...
    public void testBidAndOfferAndLastPriceUpdateToMultipleConsumer() throws Exception {
        try (
                JpInternalConsumer jpStride = new JpInternalConsumer(BID_OFFER_LASTPRICE_INTERNAL_PORT, "JP Stride");
                JpInternalConsumer jpAlgo = new JpInternalConsumer(BID_OFFER_LASTPRICE_INTERNAL_PORT, "JP Algo");
                JpInternalConsumer jpPrimeServices = new JpInternalConsumer(BID_OFFER_LASTPRICE_INTERNAL_PORT, "JP Prime Services");
        ) {
